...
/*   Updated: 2020/09/21 22:37:31 by bshang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (!(h->txtr->arm = set_textures(h, h->txtr->arm, PATH_ARMS_C)))
		return (ERR_MEM);
	if (!(h->txtr->sky = set_textures(h, h->txtr->sky, PATH_SKY_C)))
		return (ERR_MEM);
	return (0);
}
...
int		play(t_head *h)
{
	if (h->scan->bonus_f == 2 && h->scan->save_f != 1)
		system(PATH_MUS_C);
